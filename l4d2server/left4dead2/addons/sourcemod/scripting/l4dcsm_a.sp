...
{	
	...
	// Enable Cvar
	new Handle:Enabled = CreateConVar("l4d_csm_enable", "1", "切换 CSM 插件功能", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 1.0);
	g_bEnabled = GetConVarBool(Enabled);
	HookConVarChange(Enabled, _ConVarChange__Enable);
	
	// Admin cvar
	new Handle:AdminsOnly = CreateConVar("l4d_csm_admins_only", "0","更改 sm_csm 命令的访问权限 0 = 公共 1 = 仅限管理员.",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0, true, 1.0);
	g_bAdminsOnly = GetConVarBool(AdminsOnly);
	HookConVarChange(AdminsOnly, _ConVarChange__AdminsOnly);
	
	// Announce cvar
	new Handle:Announce = CreateConVar("l4d_csm_announce", "1","Toggles the announcement of sm_csm command availability\n切换 sm_csm 命令的公告.",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0, true, 1.0);
	g_bAnnounce = GetConVarBool(Announce);
	HookConVarChange(Announce, _ConVarChange__Announce);
	
	// Survivor cvar
	new Handle:SurvivorModels = CreateConVar("l4d_csm_survivor_access", "1","1 = change to character (you become a clone). 2 = change model (look like the new character, but still your original character). 3 = Allow both 1 and 2.\n1 = 更改为人物（您成为克隆人）2 = 更改模型（看起来像新人物，但仍然是您的原始人物）3 = 同时允许 1 和 2",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0, true, 3.0 );
	g_iSurvivorModels = GetConVarInt(SurvivorModels);
	HookConVarChange(SurvivorModels, _ConVarChange__SurvivorModels);
	
	// Infected cvar
	new Handle:InfectedModels = CreateConVar("l4d_csm_infected_access", "0", "1 = Can change character only. 2 = Change model only (look like the new character, but still your original character). 3 = Anybody can change character or model.\n跟前者同理", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 3.0);
	...
	{
		// L4D1 Survivors cvar
		new Handle:L4D1Survivors = CreateConVar("l4d_csm_l4d1_survivors", "0","Toggles access to L4D1 Survivors in L4D2. Set this to 1 if it's a L4D2 campaign, else if its a L4D1 Campaign, set this to 2 or glitches will occur\n切换对 L4D2 中 L4D1 幸存者的访问",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0, true, 2.0);
		...
		new Handle:CommonModels = CreateConVar("l4d_csm_common_infected_models_access", "0", "Toggles access to Common Infected models\n切换特感模型的访问", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 3.0);
		g_bCommonModels = GetConVarBool(InfectedModels);
		HookConVarChange(CommonModels, _ConVarChange__CommonModels);
	}
	
	// Change Limit Cvar
	new Handle:ChangeLimit = CreateConVar("l4d_csm_change_limit", "9999","Sets the number of times clients can change their character per life/map.\n设置客户端在每次生命/地图中可以更改其人物的次数(不用管awa)",FCVAR_PLUGIN|FCVAR_SPONLY,true, 0.0);
	...
	if (IsClientInGame(client)) 
	{
		if (GetClientTeam(client) == TEAM_SURVIVORS && g_bAnnounce)
		{
			switch (g_iSurvivorModels) 
			{
				...
				PrintToChat(client, "\x03[CSM]\x01 聊天框输入 \x05!csm\x01 更改您的模型 (外观).");
				case CSM_SURVIVOR_CHARACTER_ONLY:
				PrintToChat(client, "\x03[CSM]\x01 聊天框输入 \x05!csm\x01 更改您的人物 (外观、声音和人物图片).");
				case CSM_SURVIVOR_MODEL_AND_CHARACTER:
				PrintToChat(client, "\x03[CSM]\x01 聊天框输入 \x05!csm\x01 更改您的人物 (外观、声音和人物图片) 或模型 (外观).");
			}
			...
			switch (g_iInfectedModels) 
			{
				...
				PrintToChat(client, "\x03[CSM]\x01 聊天框输入 \x05!csm\x01 您的人物更改为特感 (外观、技能和声音).");
				case CSM_INFECTED_CHARACTER_AND_MODEL:
				PrintToChat(client, "\x03[CSM]\x01 聊天框输入 \x05!csm\x01 您的人物 (外观、技能和声音) 或模型（外观）更改为特感.");
			}
		}
	}
}
...
public Action:InitiateMenuAdmin(client, args) 
{
	if (client == 0) 
	{
		ReplyToCommand(client, "[CSM] 人物选择菜单只能在游戏中使用.");
		return;
	}
	if ((g_iChangeLimit == 0) || !g_bEnabled) 
	{
		ReplyToCommand(client, "[CSM] 人物选择菜单当前处于禁用状态.");
		return;
	}
	if (g_iSurvivorModels == 0 && g_iInfectedModels == 0 && !g_bCommonModels)
	{
		ReplyToCommand(client, "[CSM] 人物选择菜单没有可选项.");
		return;
	}
...
public Action:ShowMenu(client, args) 
{
	...
	{
		ReplyToCommand(client, "[CSM]人物选择菜单只能管理员使用.");
		return;
	}
	if (GetClientTeam(client) != TEAM_SURVIVORS)
	{
		ReplyToCommand(client, "[CSM] 人物选择菜单只能幸存者使用.");
		return;
	}
	if (g_iSurvivorModels == 0 && g_iInfectedModels == 0 && !g_bCommonModels)
	{
	...
	{
		ReplyToCommand(client, "[CSM] 死亡之后无法使用人物选择菜单!");
		return;
	}
	if (g_iClientChangeLimit[client] < 1) 
	{
		ReplyToCommand(client, "[CSM] 在你重新复活之前无法更改你的人物.");
		return;
	}
	...
	if (g_bAnnounce)
	{
		if (g_iClientChangeLimit[param1] <= CSM_CHANGE_MESSAGE) 
		{
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01! You cannot change again until you respawn.", g_sCharacters[Survivor]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01! You only have \x051\x01 change left.", g_sCharacters[Survivor]);
			else
			PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01! You have \x05%i\x01 changes left.", g_sCharacters[Survivor], g_iClientChangeLimit[param1]);
		} 
		else 
		{
			PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01!", g_sCharacters[Survivor]);
		}
	}
}
...
stock SetUpSurvivorModel(param1, Survivor) 
{
	...
	if (g_bAnnounce)
	{
		if (g_iClientChangeLimit[param1] <= CSM_CHANGE_MESSAGE)
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01! You cannot change again until you respawn.", g_sCharacters[Survivor+8]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01! You only have \x051\x01 change left.", g_sCharacters[Survivor+8]);
			else
		PrintToChat(param1, "\x03[CSM]\x01 您的模型现在是 \x04%s\x01! You have \x05%i\x01 changes left.", g_sCharacters[Survivor+8], g_iClientChangeLimit[param1]);
		else
		PrintToChat(param1, "\x03[CSM]\x01 您的模型现在是 \x04%s\x01!", g_sCharacters[Survivor+8]);
	}
}
...
stock SetUpInfected(param1, Infected)
{
	...
	if (g_bAnnounce)
	{
		...
		{
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01! You cannot change again until you respawn.", g_sCharacters[Infected]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01! You only have \x051\x01 change left.", g_sCharacters[Infected]);
			else
			PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01! You have \x05%i\x01 changes left.", g_sCharacters[Infected], g_iClientChangeLimit[param1]);
		} 
		else 
		{
			PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01!", g_sCharacters[Infected]);
		}
	}
...
stock SetUpInfectedModel(param1, Infected)
{
	...
	if (g_bAnnounce)
	{
		...
		{
			if (g_iClientChangeLimit[param1] == 0)
				PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01! You cannot change again until you respawn.", g_sCharacters[Infected+8]);
			else if (g_iClientChangeLimit[param1] == 1)
				PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01! You only have \x051\x01 change left.", g_sCharacters[Infected+8]);
			else
			PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01! You have \x05%i\x01 changes left.", g_sCharacters[Infected+8], g_iClientChangeLimit[param1]);
		}
		else
		{
			PrintToChat(param1, "\x03[CSM]\x01 您的模型更改为 \x04%s\x01!", g_sCharacters[Infected+8]);
		}
	}
}
