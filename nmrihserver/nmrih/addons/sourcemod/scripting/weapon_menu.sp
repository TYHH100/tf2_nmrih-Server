...
#define MAX_CONFIG_WEAPONS 1024
...
new Handle:g_hAmmoLimit = INVALID_HANDLE;
...
{
	CreateConVar("sv_nmrih_weaponmenu_version", Plugin_Version, "This is the version of the Weapon Menu the server is running.", FCVAR_PLUGIN|FCVAR_NOTIFY|FCVAR_SPONLY);
	g_hPluginEnabled = CreateConVar("sv_nmrih_weaponmenu_enabled", "1", "Turns the plugin on or off. (1 = Enabled : 0 = Disabled)", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 1.0);
	...
	g_hAmmoLimit = CreateConVar("sv_weaponmenu_ammo_limit", "1", "Sets the maximum amount of times ammo can be selected.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	HookConVarChange(g_hAmmoLimit, OnSettingsChange);
	g_hWeaponAdminOverride = CreateConVar("sv_weaponmenu_admin_override", "0", "Discards all weapon limitations for an Admin.", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0);
	...
	g_iAmmoLimit = GetConVarInt(g_hAmmoLimit);

	g_bAdminOverride = GetConVarBool(g_hWeaponAdminOverride);

	RegAdminCmd("sm_guns", Command_WeaponMenu, ADMFLAG_GENERIC);
...
public OnSettingsChange(Handle:hConvar, const String:sOldValue[], const String:sNewValue[])
{
	...
	if (hConvar == g_hAmmoLimit) g_iAmmoLimit = StringToInt(sNewValue);
}
...
public Action:Command_WeaponMenu(iClient, iArgs)
{
	if (IsPlayerAlive(iClient))
	{
		...
		SetMenuTitle(hMenu, "[NMRiH] Weapon Menu|武器菜单");

		AddMenuItem(hMenu, "0", "Rifles|步枪", ((g_iRifleLimit > 0) && (g_iRifleCount[iClient] < g_iRifleLimit)) && (ItemQuantity("rifle") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "1", "SubMachineGuns|冲锋枪", ((g_iSubMachineGunLimit > 0) && (g_iSubMachineGunCount[iClient] < g_iSubMachineGunLimit)) && (ItemQuantity("smg") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "2", "Shotguns|散弹枪", ((g_iShotgunLimit > 0) && (g_iShotgunCount[iClient] < g_iShotgunLimit)) && (ItemQuantity("shotgun") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "3", "Pistols|手枪", ((g_iPistolLimit > 0) && (g_iPistolCount[iClient] < g_iPistolLimit)) && (ItemQuantity("pistol") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "4", "Tools|工具", ((g_iToolLimit > 0) && (g_iToolCount[iClient] < g_iToolLimit)) && (ItemQuantity("tool") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "5", "Melee|近战", ((g_iMeleeLimit > 0) && (g_iMeleeCount[iClient] < g_iMeleeLimit)) && (ItemQuantity("melee") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "6", "Medical Supplies|医疗用品", ((g_iMedicalLimit > 0) && (g_iMedicalCount[iClient] < g_iMedicalLimit)) && (ItemQuantity("medic") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "7", "Explosives|爆炸物", ((g_iExplosiveLimit > 0) && (g_iExplosiveCount[iClient] < g_iExplosiveLimit)) && (ItemQuantity("explo") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
		AddMenuItem(hMenu, "8", "AMMO|Test", ((g_iAmmoLimit > 0) && (g_iAmmoCount[iClient] < g_iAmmoLimit)) && (ItemQuantity("ammo") > 0) ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);

		DisplayMenu(hMenu, iClient, MENU_TIME_FOREVER);
	} else
	PrintToChat2(iClient, "%s 你必须活着才能使用该指令!", sError);
	...
	switch (action)
	{
		...
		case MenuAction_Select:
		{
			...
			switch (StringToInt(sInfo))
			{
				...
				{
					hMenu = CreateMenu(Weapon_Menu);
					SetMenuTitle(hMenu, "武器菜单: Ammo");

					for (new i; i < g_iWeapons; i++)
					{
						if (StrContains(g_sWeaponEntityCategory[i], "ammo", false) == -1) continue;
...
public Weapon_Menu(Handle:hHandle, MenuAction:action, param1, param2)
{
	switch (action)
	{
		...
		case MenuAction_Select:
		{
			...
			PrintToChat2(param1, "%s 你选择了 \x04%s\x01.", sNotice, g_sWeaponDisplayNames[StringToInt(sInfo)]);
		}
	}
...
Config_LoadWeapons() // Originally, this was from TwistedPanda's BuildWars : It helped alot!!
{
	...
	if (FileToKeyValues(hKeyValue, sPath))
	{
		...
		PrintToServer("[NMRiH] 武器菜单: 已加载 %d 武器", g_iWeapons);
	} else
	{
		CloseHandle(hKeyValue);
		SetFailState("[NMRiH] 武器菜单: 无法找到: %s", sPath);
	}
}
