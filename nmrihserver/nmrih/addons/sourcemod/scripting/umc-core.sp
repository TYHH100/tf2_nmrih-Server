...
new Handle:cvar_zpomaxrnds = INVALID_HANDLE; // ZPS Objectivez
	...
	if (NeedRunoff(vM))
	{
		...
		if (remaining_runoffs == 0 || prev_vote_count == 2)
		{
			...
			else if (stored_fail_action == RunoffFailAction_Nothing)
			{
				...
				PrintToChatAll("\x03[UMC]\x01 %t (%t)", "Vote Failed", "Vote Win Percentage", percentage, total_votes);
...
public Handle_MapVoteWinner(Handle:vM, const String:info[], const String:disp[], Float:percentage)
{
	...
	{
		PrintToChatAll("\x03[UMC]\x01 %t %t (%t)", "End of Map Vote Over", "Map Extended",
	...
	{
		PrintToChatAll("\x03[UMC]\x01 %t %t (%t)", "End of Map Vote Over", "Map Unchanged",
			"Vote Win Percentage", percentage, total_votes);
	...
	{
		PrintToChatAll("\x03[UMC]\x01 %t %t (%t)", "End of Map Vote Over", "End of Map Vote Map Won",
							...
							Float:percentage)
{
	...
	else //Otherwise, we pick a random map from the category and set that as the next map.
	{
		...
		PrintToChatAll("\x03[UMC]\x01 %t %t (%t)", "End of Map Vote Over", "End of Map Vote Group Won",
